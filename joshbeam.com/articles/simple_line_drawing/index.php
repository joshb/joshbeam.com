<?php
	...
	print_html_header("Tutorial - Introduction to Software-based Rendering: Simple Line Drawing");
?>
...
<?php
	for($x = 0; $x < 8; $x++) {
		?>
			<td style="border: 0"><?php echo $x ?></td>
		<?php
	}
?>
		</tr>
<?php
	for($y = 0; $y < 8; $y++) {
		?>
		<tr>
			<td style="border: 0; background-color: #555555; color: white"><?php echo $y ?></td>
		<?php
		for($x = 0; $x < 8; $x++) {
			?>
			<td<?php
			if($y == 2 && $x == 3) echo " style=\"font-weight: bold\"";
			?>><?php echo ($y * 8) + $x ?></td>
			<?php
		}
		?>
                     ...
                     const Color &amp;color2, float x2, float y2)
{
	...
	<ul>
		<li><strong><a href="linedrawing-1.tar.gz">linedrawing-1.tar.gz</a> (source tarball)</strong></li>
		<ul>
